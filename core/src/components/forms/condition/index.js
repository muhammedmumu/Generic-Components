...
const Condition = React.memo(
  ({ condition, children, values, field }) => {
    ...
    const setters = conditionResult.set
      ? [conditionResult.set]
      : conditionResult.sets;
    ...
    useEffect(() => {
      if (
        setters &&
        setters.length > 0 &&
        (state.initial || !isEqual(setters, state.sets))
      ) {
        setters.forEach((setter, index) => {
          if (
            setter &&
            (state.initial || !isEqual(setter, state.sets[index]))
          ) {
            setTimeout(() => {
              formOptions.batch(() => {
...
Condition.propTypes = {
  condition: PropTypes.oneOfType([
    PropTypes.shape(conditionsProps),
    PropTypes.arrayOf(PropTypes.shape(conditionsProps)),
  ]),
