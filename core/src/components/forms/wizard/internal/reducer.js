    ...
    schema = [
      ...schema,
      {
        ...
        primary:
          ...
          !field.substepOf ||
          field.substepOf !== schema[schema.length - 1].substepOf,
      },
    ];
...
const handleNext = (state, nextStep, formOptions, fields) => {
  ...
  const shouldInsertStepIntoHistory = state.prevSteps.includes(
    state.activeStep
  );
  ...
  return {
    ...
    activeStep: nextStep,
    prevSteps: shouldInsertStepIntoHistory
      ? state.prevSteps
      : [...state.prevSteps, state.activeStep],
    activeStepIndex: newActiveIndex,
    maxStepIndex:
      newActiveIndex > state.maxStepIndex ? newActiveIndex : state.maxStepIndex,
...
export const findCurrentStep = (activeStep, fields) =>
...
const jumpToStep = (state, index, valid, fields, crossroads, formOptions) => {
  ...
  if (clickOnPreviousStep) {
    ...
    const includeActiveStep = state.prevSteps.includes(
      state.activeStep,
      fields
    );
    ...
    const currentStepHasStepMapper = DYNAMIC_WIZARD_TYPES.includes(
      typeof currentStep.nextStep
    );

    const hardcodedCrossroads = crossroads;
    const dynamicStepShouldDisableNav =
      newState.isDynamic && currentStepHasStepMapper;
    ...
    if (dynamicStepShouldDisableNav && !hardcodedCrossroads) {
      ...
      updatedState = {
        ...updatedState,
        prevSteps: newState.prevSteps.slice(
          0,
          indexOfCurrentStep + INDEXING_BY_ZERO
        ),
        maxStepIndex:
          newState.prevSteps.slice(0, indexOfCurrentStep + INDEXING_BY_ZERO)
            .length - INDEXING_BY_ZERO,
      };
    }
