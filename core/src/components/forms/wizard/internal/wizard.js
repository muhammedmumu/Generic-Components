...
import useFormApi from "../../hooks/use-form-api.js";
...
const Wizard = ({
  ...
  const [state, dispatch] = useReducer(reducer, {
    ...
    isDynamic:
      isDynamic ||
      fields.some(({ nextStep }) =>
        DYNAMIC_WIZARD_TYPES.includes(typeof nextStep)
      ),
    loading: true,
  });
  ...
  const jumpToStep = (index, valid) =>
    dispatch({
      type: "jumpToStep",
      payload: { index, valid, fields, crossroads, formOptions },
    });
  ...
  const handleNext = (nextStep) =>
    dispatch({
      type: "handleNext",
      payload: { nextStep, formOptions, fields },
    });

  const setPrevSteps = () =>
    dispatch({ type: "setPrevSteps", payload: { formOptions, fields } });
