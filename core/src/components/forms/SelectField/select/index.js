...
const getSelectValue = (stateValue, simpleValue, isMulti, allOptions) =>
  simpleValue
    ? allOptions.filter(({ value }) =>
        isMulti ? stateValue.includes(value) : isEqual(value, stateValue)
      )
    : stateValue;
...
const Select = ({
      ...
      if (isMounted.current) {
        if (!noValueUpdates) {
          if (value && Array.isArray(value)) {
            ...
            value &&
            !data.find(({ value: internalValue }) => internalValue === value)
          ) {
            onChange(undefined);
          }
        }
  ...
  useEffect(() => {
    if (state.isInitialLoaded) {
      if (
        !noValueUpdates &&
        value &&
        !propsOptions.map(({ value }) => value).includes(value)
      ) {
        onChange(undefined);
      }
  ...
  const renderNoOptionsMessage = () =>
    Object.values(state.promises).some((value) => value)
      ? () => updatingMessage
      : () => noOptionsMessage;
  ...
  const onInputChange = (inputValue) => {
    if (
      inputValue &&
      loadOptions &&
      state.promises[inputValue] === undefined &&
      props.isSearchable
    ) {
  ...
  const selectValue = pluckSingleValue
    ? isMulti
      ? value
  ...
  return (
    <SelectComponent
      ...
      onChange={(option) =>
        handleSelectChange(option, simpleValue, isMulti, onChange)
      }
